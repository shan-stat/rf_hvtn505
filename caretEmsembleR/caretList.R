##### Attention #####
# In this file, caretList() and predict.caretList() functions were modified #
# Other functions are same as the original file #
# modified caretList(): is available to fit candidate learners on different training sets #
# modified predict.caretList(): is to select case as positive #
...
caretList <- function(
  ...
  ###############################################################################
  # !!! The following code was removed to use duplicated candidate learners !!! #
  ###############################################################################
  ...
  if(is.null(trControl$index)){
    ###################################################################################
    # !!! The following code was modified to catch a target variables as only 'Y' !!! #
    ###################################################################################
    #target <- extractCaretTarget(...)     # before
    target <- global_args[[1]]$data[,'Y']  # after
    trControl <- trControlCheck(x=trControl, y=target)
  }
  
  #######################################################################################
  # !!! The following code was modified to be appropriate for the modified stacking !!! #
  #######################################################################################
  #Squish trControl back onto the global arguments list
  #global_args[["trControl"]] <- trControl # before
  for(i in 1:length(global_args)){         # after
    global_args[[i]][["trControl"]] <- trControl
  }
  ...
  preds <- pbsapply(object, function(x){
    ...
    if (type=="Classification"){
      if(x$control$classProbs){
        ...
        ######################################################################
        # !!! The following code was modified to select case as positive !!! #
        ######################################################################
        #caret::predict.train(x, type="prob", newdata=newdata, ...)[, getBinaryTargetLevel()] # before
        caret::predict.train(x, type="prob", newdata=newdata, ...)[, 'case']                  # after   
      } else{
