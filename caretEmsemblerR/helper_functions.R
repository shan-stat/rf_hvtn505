##### Attention #####
# In this file, makePredObsMatrix() function was only modified#
...
makePredObsMatrix <- function(list_of_models){
  ...
  if (type=="Classification"){
    ...
    ######################################################################
    #positive <- levels(modelLibrary$obs)[getBinaryTargetLevel()]                  # before
    positive <- levels(modelLibrary$obs)[which(levels(modelLibrary$obs)=='case')]  # after
