##### Attention #####
# In this fiel, predict.caretStack() was only modified #
...
predict.caretStack <- function(
  ...
  ###########################################################################################
  # !!! The following code was modified to use predict.caretList() instead of predict() !!! #
  ##########################################################################################
  #preds <- predict(object$models, newdata=newdata, na.action=na.action)           # before
  preds <- predict.caretList(object$models, newdata=newdata, na.action=na.action)  # after
  if(type == "Classification"){
    ...
    if(inherits(out, c("data.frame", "matrix"))){
      ...
      ######################################################################
      #est <- out[, getBinaryTargetLevel(), drop = TRUE] # before
      est <- out[, 'case', drop = TRUE]                  # after
    } else{
      est <- out
    }
  } else{
