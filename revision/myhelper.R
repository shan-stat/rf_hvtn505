...
screen.dat.index <- function(Y, X, fit.set = c('no','antibody','tcell','all'), screen.method = c('no','lasso'), obsWeights){
  ...
  dat.X <- list( case = dat[dat$Y == 1, c(FALSE, screen.index)], control = dat[dat$Y == 0, c(FALSE, screen.index)])
