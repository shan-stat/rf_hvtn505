...
random.strata.splits <- function (Y, X, strata, seed){
  set.seed(seed)
  case.val.idx <- sample(which(Y==1), size=sum(Y==1)/5, replace=FALSE)
  case.val.freq <- table(strata[case.val.idx])
  case.total.freq <- table(strata[which(Y==1)])
  ratio <- c()
...
# Stacking #
get.st.cvauc = function(Y, X, obsWeights, strata, var.index, method, seed=1, mc.cores=mc.cores){
