...
weights <- dat.505$wt
Y <- dat.505$case
strata <- dat.505$hg_strata


# Vaccine arm (150 observations) #
vaccinees <- cbind.data.frame( Y, weights, strata, X ) %>% filter( trt == 1 ) %>% select( -trt )
rownames(vaccinees) <- 1:150
X_none <- cbind.data.frame( Y, weights, strata, X_full_none ) %>% filter( trt == 1 ) %>% select( -trt, -Y, -weights, -strata )
Y_vaccine <- vaccinees$Y
weights_vaccine <- vaccinees$weights
strata_vaccine <- vaccinees$strata
X_vaccine <- vaccinees %>% select( -Y, -weights, -strata )
