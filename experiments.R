...
for(j in 1:length(can.set)){
  print(j)
  # Generating data #
  res.screen <- screen.dat.index(Y=Y_vaccine, X=X_vaccine, obsWeights=weights_vaccine, fit.set=can.set[j], screen.method=screening)
  if(covariates == 'no'){
    # Without covariates #
...
pred.mat
round(apply(pred.mat,2,mean),3); round(apply(pred.mat,2,sd),3)
...
dat.X <- screen.dat.index(Y=Y_vaccine, X=X_vaccine, obsWeights=weights_vaccine, fit.set='all', screen.method='no')$dat

# Specifying screened variables index #
# For all markers #
screen.var.all <- screen.dat.index(Y=Y_vaccine, X=X_vaccine, obsWeights=weights_vaccine, fit.set='all', screen.method='lasso')$screen.index.var

# For t cell markers #
screen.var.tcell.temp <- screen.dat.index(Y=Y_vaccine, X=X_vaccine, obsWeights=weights_vaccine, fit.set='tcell', screen.method='lasso')$screen.index.var
...
screen.var.antibody.temp <- screen.dat.index(Y=Y_vaccine, X=X_vaccine, obsWeights=weights_vaccine, fit.set='antibody', screen.method='lasso')$screen.index.var
