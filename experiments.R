...
for(j in 1:length(can.set)){
  ...
  if(can.set[j]=='no' & covariates=='no'){
    ...
    for( i in 1:100 ){
      ...
      seed <- i
      # RF #
      res.temp <- get.rf.cvauc(Y=dat[,'Y'], X=select(dat,-"Y"), obsWeights=weights_vaccine, method='RF', ipw=TRUE, seed=seed)
      pred.mat[i, can.set[j]] <- mean( res.temp )
    }
  }
}
pred.mat
round(apply(pred.mat,2,mean),3)
...
for(j in 1:length(can.set)){
  ...
  for( i in 1:100 ){
    ...
    seed <- i
    # GLM #
    res.temp <- get.glm.cvauc(Y=dat[,'Y'], X=select(dat,-"Y"), obsWeights=weights_vaccine, ipw=TRUE, seed=seed)
    pred.mat[i, can.set[j]] <- mean( res.temp )
  }
}
pred.mat
...
for(j in 1:length(stacking.set)){
  ...
  for( i in 1:100 ){
    ...
    # Stacking (Pearson correlation coefficients; for specific random seed 1) #
