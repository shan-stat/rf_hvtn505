...
random.strata.splits <- function (Y, X, strata, seed){
  ...
  case.total.freq <- table(strata[which(Y==1)]) # strata information for the entire cases (25 cases, not the sampled cases)
  
  # compute the ratio of that how many cases are sampled in each stratum #
  # e.g. strata1 - 4cases/20controls. 2cases are randomly sampled, then the ratio is 2/4=0.5 #
  ratio <- c()
  ...
  # e.g. if the ratio is 0.5, then 20*0.5 controls will be sampled in strata 1 #
  control.val.idx <- c()
...
get.st.cvauc.random = function(Y, X, obsWeights, strata, var.index, method, seed=1, mc.cores=mc.cores){
  ...
  cv.aucs <-  mclapply( splits, function(split){
    ...
    # but, the two results are a bit different (their hyperparameter setting is the same) #
...
### Variability of CV-AUC estimates of stacking by using bootstrap ###
...
### 2. Specifying lasso screened variables index (e.g. RF trained on T cell, GLM trained on Antibody) ###
# For all markers #
...
res=sapply(seeds, simplify="array", function (seed) {
  ...
  # Bootstrap based on starat information #
  set.seed(seed)
...
### Variability of CV-AUC estimates of RF and GLM by using bootstrap ###
### 0. Pre-processing HVTN 505 dataset ###
data( 'dat.505', package = 'HVTN505' )
dat.505 <- subset(dat.505, trt==1) # only treatment
suppressWarnings( data( 'var.super', package = 'HVTN505' ) )
